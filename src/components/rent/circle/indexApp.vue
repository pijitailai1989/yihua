      ...
      <div>
        <div class="circle_session" ref="circle_session">
          ...
          <div class="announce" v-show="imgs" >
            <div v-for="(item,key) in announce" :key="key" class="h8" @click="goTo(item.guid)">
              <b>【{{item.typeName}}】</b>
          ...
          <div class="show filterContent" v-if="show" @click="cancel">
            <div @click.stop="''">
              <div class="flex_row" ontouchmove='return false'>
                <div class="flex_row_item" :class="{allActive:allActive[0]}" :style="allActive[0]?`color:${mainColor}`:``" @click="setActive(0)">
                  <span v-show="region==''">区域</span>
          ...
          <!---->
          <yd-pullrefresh :callback="loadTop" ref="pullrefreshDemo">
    ...
    if (info.userId) {
      this.getUserInfo(
        ...
        res => {
          ...
          this.circleType = this.getFilterType()
          this.getList();
          this.getbbsType();
        }
      );
  ...
  methods: {
    ...
    turnTo(e) {
      ...
      if(this.userAgent.isAndroid){
        ...
        toIOSApp(function (bridge) {
          ...
          bridge.callHandler('circleLink', {'service':`/#${e}`}, function responseCallback(responseData) {
      ...
      this.xhr
        ...
        .then(res => {
          if (res.data.code == 1) {
            ...
            console.log(this.parameter.userId,this.$route.query.userId)
    ...
    getFilterType(){
      ...
      if(this.userInfo.organType == 1){
        type = 1;
      }else if(this.userInfo.organType == 3 || this.userInfo.organType == 4){
        type = 3;
    ...
    getbbsType(type, index, name, id, data) {
      data = data ? data : {};
      data.circleType = this.getFilterType();
    ...
    getList(data, cb) {
      ...
      data.token = this.$route.query.token
        ? this.$route.query.token
        : this.getkey;
      data.userId = this.$route.query.token
        ? this.$route.query.userId
        : this.userInfo.guid;
      data.pageSize = this.pageSize ? this.pageSize : 15;
      data.circleType = this.getFilterType();
      if(this.bbsType1!=null) data.bbsType = this.bbsType1;
