<template>
  <yd-layout class="edit">
    <yd-navbar height=".88rem" slot='navbar' v-show="!userAgent.isWX" v-title="'发布'" title="发布" :bgcolor="mainColor"
               color="#fff">
    ...
    <div class="to-select">
      ...
      <div class="buttonBox">
        <yd-button size="large" class="send" @click.native="sendPost" :bgcolor="mainColor" color='#fff' type="primary">
          立即发布
        </yd-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    data() {
      let userInfo, organInfo, mainColor, endTime,TypeListCircle;
      endTime = this.$today();
      if (this.$route.query.token) {
        this.getUserInfo(res => {
          ...
          if (userType == 1) {
            this.TypeListCircle = 1;
          } else if (userType == 5) {
            this.TypeListCircle = 2;
          } else {
            this.TypeListCircle = 3
          }
    ...
    methods: {
      ...
      sendPost() {
        ...
        console.log(this.TypeListCircle,'111111111111')
        data.circleType = this.TypeListCircle;
