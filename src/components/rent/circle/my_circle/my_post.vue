<template>
  <yd-layout class="editBox">
    ...
    <ul slot='top'>
      <li v-for="(item,index) in tabs" @click="selectTab(index)" :key="index" :style="`color:${showTab==index?mainColor:'#333'};border-color:${showTab==index?mainColor:'transparent'}`" v-bind:class="showTab==index?'li-active':''">{{item}}
      </li>
    </ul>
