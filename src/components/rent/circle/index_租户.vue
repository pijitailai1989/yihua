<template>
  <div class="circle"  v-title="'八爪圈'">
          ...
          <yd-pullrefresh :callback.stop.prevent="loadList" ref="pullrefreshDemo">
            <yd-slider autoplay="3000" v-show="!isFilter" :loop="false">
              <yd-slider-item>
                <yd-grids-group :rows="4">
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_03.png">
                    <span slot="text">办公器械</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_05.png">
                    <span slot="text">岗位培训</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_07.png">
                    <span slot="text">商标注册</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_09.png">
                    <span slot="text">投资担保</span>
                  </yd-grids-item>
                  <!--第二行-->
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_15.png">
                    <span slot="text">物流快递</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_16.png">
                    <span slot="text">广告印刷</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_17.png">
                    <span slot="text">设计策划</span>
                  </yd-grids-item>
                  <yd-grids-item class="items_navbar">
                    <img slot="icon" src="../../../assets/images/rent/circle/navbar_18.png">
            ...
            <div class="flex_row">
              <div class="flex_row_item" :class="{allActive:allActive[0]}" :style="allActive[0]?`color:${mainColor}`:``"   @click="setActive(0)">全部
                <img v-if="!allActive[0]" src="../../../assets/images/rent/circle/arrow_bottom.png">
                <img v-else src="../../../assets/images/rent/circle/arrow_top.png">
              </div>
              <div class="flex_row_item" :class="{allActive:allActive[1]}" :style="allActive[1]?`color:${mainColor}`:``"   @click="setActive(1)">附近
                <img v-if="!allActive[1]" src="../../../assets/images/rent/circle/arrow_bottom.png" alt="">
                <img v-else src="../../../assets/images/rent/circle/arrow_top.png" alt="">
              </div>
              <div class="flex_row_item" :class="{allActive:allActive[2]}" :style="allActive[2]?`color:${mainColor}`:``"  @click="setActive(2)">综合排序
                <img v-if="!allActive[2]" src="../../../assets/images/rent/circle/arrow_bottom.png">
                <img v-else src="../../../assets/images/rent/circle/arrow_top.png">
              </div>
            </div>
            <!-- 公告 -->
            ...
            <yd-infinitescroll :callback="loadListTop" ref="infinitescrollDemo">
              ...
              <svg xmlns="http://www.w3.org/2000/svg" slot="loadingTip" viewBox="0 0 100 100"
                   preserveAspectRatio="xMidYMid" class="lds-ellipsis">
                <circle cx="84" cy="50" r="5.07611" fill="#f3b72e">
                ...
                <circle cx="66.7412" cy="50" r="10" fill="#E8574E">
                ...
                <circle cx="32.7412" cy="50" r="10" fill="#43A976">
                  <animate attributeName="r" values="0;10;10;10;0" keyTimes="0;0.25;0.5;0.75;1"
                           keySplines="0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1" calcMode="spline" dur="1.7s"
                ...
                <circle cx="16" cy="50" r="4.92389" fill="#304153">
                  <animate attributeName="r" values="0;10;10;10;0" keyTimes="0;0.25;0.5;0.75;1"
                           keySplines="0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1;0 0.5 0.5 1" calcMode="spline" dur="1.7s"
                           repeatCount="indefinite" begin="0s"></animate>
                  <animate attributeName="cx" values="16;16;50;84;84" keyTimes="0;0.25;0.5;0.75;1"
    ...
    <router-link to="/rent/circle/rent_edit" class="toEdit">
      <img src="../../../assets/images/rent/circle/edit.png">
    </router-link>
    <!--实际筛选内容-->
    <div class="filterOuter" v-on:click="closeFilter" v-show="isFilter">
      <div class="filterBox" v-on:click.stop.prevent='prevent()'>
        <!--全部筛选部分-->
        <div class="allActive1" v-show="allActive[0]">
          <div class="left">
            <div v-for="(item,key) in filterAllData[0][0]" :key="item.id"
            v-bind:style="showLeftSelected==key?`background:#f5f5f5;color:${mainColor}`:''" @click="showThis(key)">{{item.title}}</div>
          </div>
          <div class="right">
            <yd-radio-group  v-model="filter1">
              <yd-radio class="yd-radio" :style="key1==filter1?`background:#f1f1f1`:'background:#fff'"  v-for="(item1,key1) in filterList" @click.native="choose(key1,item1)" :key="item1.id" :val="key1">
        ...
        <div class="allActive1" v-show="allActive[1]">
          <div class="left">
            <div v-for="(item,key) in filterAllData[0][1]" v-if="key<1" :key="item.id"
            v-bind:style="showLeftSelected==key?`background:#f5f5f5;color:${mainColor}`:''" @click="showThis1(key,item)">{{item.title}}</div>
            <div v-for="(item,key) in filterAllData[0][1]" v-if="key>=1" :key="item.id"
            v-bind:style="showLeftSelected==key?`background:#f5f5f5;color:${mainColor}`:''" @click="showThis1(key,item.name)">{{item.name}}</div>
          </div>
          <div class="right">
            <yd-radio-group  v-model="filter1">
              <yd-radio class="yd-radio" :style="key1==filter1?`background:#f1f1f1`:'background:#fff'"  v-for="(item1,key1) in filterList1" @click.native="choose(key1,item1)" :key="item1.id" :val="key1">
                <span class="yd_title">{{item1.title}}</span>
                <span class="yd_text">{{item1.count}}</span>
              </yd-radio>
            </yd-radio-group>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      listItemArr: [
        {
          title: "[程序安全]最近片区安全状况如何",
          content: "年底了,各种安全事件频发阶段，大家有什么方式来...",
          headerUrl: require("../../../assets/images/rent/circle/header.jpg"),
  ...
  methods: {
    showThis(index) {
      if (index > 0) {
        ...
        this.filterList = this.filterAllData[0][0][index].subArr;
      } else {
    ...
    choose(i, item) {
      this.filter1 = i;
      this.$dialog.toast({ mes: `您选择了${item.title}`, timeout: 1000 });
      this.closeFilter();
    },
    loadList() {
  ...
  mounted() {
    ...
    this.filterList = this.filterAllData[0][0][1].subArr;
    this.filterList1 = this.filterAllData[0][1][0].subArr;
    // 假数据
    for (var i = 0; i < 10; i++) {
      this.listItemArr.push(this.listItemArr[0]);
    }
  }
};
</script>
