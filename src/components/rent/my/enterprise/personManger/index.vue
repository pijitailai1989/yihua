          ...
          <li class="child-bottom" @click="selectThis(index,no)" v-for="(item,no) in it.empList" :key="no"
            ...
            <div>
              ...
              <div class="flex font24 foCo9">
                <span class="l-center" v-if="item.post">岗位：{{item.post}}</span>
                <span  class="l-center" v-if="!item.post">岗位：暂无</span>
              </div>
            </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      sureDelDept(guid) {
        this.xhr.post(`${this.subUrl.user}/admin/deptManage/delDepartment?token=` + data.token + '&organId=' + this.organId ? this.organId : this.organInfo.guid + '&DepartmentId=' + guid).then((res) => {
          if (res.data.code == 1) {
