  ...
  <div style="height:100vh">
    <yd-layout v-show="showUser && showTable &&showMuti">
      <yd-navbar slot="navbar" :title="mTitle" height=".88rem" color="#fff" :bgcolor="mainColor">
        ...
        <!--<div class="addRightClickArea" style="color:#fff" @click="confirm" slot="right">保存</div>-->
      </yd-navbar>
      <div class="usuallyBox bb" style="padding: .1rem 0 .1rem;">
        <div class="list_item">
          <div class="label">最新进度</div>
          <input type="text" readonly maxlength="3" style="border: 0;" class="flex1" placeholder="暂无详情" v-model="progress">
        </div>
        <div class="list_item">
          <div class="label">更新时间</div>
          <input type="text" readonly  style="border: 0;" class="flex1" placeholder="暂无详情" v-model="updateTime">
        </div>
        <div class="list_item mt2" style="height: auto;align-items: flex-start;padding-bottom: .2rem">
          <div class="label mt1">备注</div>
          <yd-textarea maxlength="100" :show-counter="false" style="height: 2rem;flex:1;padding:.1rem .1rem .4rem;margin-left:.2rem;"
                       class="foCo9 radius"
                       placeholder="暂无详情" v-model="remark"></yd-textarea>
        </div>
      </div>
      <div class="mt2 bgwhite bt bb" style="padding: .3rem .24rem;">
        <div class="flex-row foCo9 bgwhite">成果详情</div>
  ...
  export default {
    data() {
      ...
      getDetail() {
        ...
        this.$ajax(`${this.subUrl.activity}/taskBill/taskProgress/progressLog`, data, res => {
