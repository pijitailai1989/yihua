      ...
      <yd-accordion-item title="帖子类型" style="margin-bottom:.2rem;background:#fff;" open>
        <div style="padding: .24rem;">
          <yd-radio-group v-model="TypeList" v-for="(item,index) in bbsList" :key="index">
            <yd-radio class="choose_item" :val="item.guid">
              {{item.dictDataValue}}
            </yd-radio>
            <yd-radio v-if="item.childDictData.length>0" class="er choose_item" v-for="(el,index) in item.childDictData" :key="index" :val="el.guid">
              {{el.dictDataValue}}
            </yd-radio>
         </yd-radio-group>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      sendPost() {
        ...
        // console.log(data.organId, this.organInfo.organId)
