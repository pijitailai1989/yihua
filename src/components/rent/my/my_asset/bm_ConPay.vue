<template>
  <div class="layout_my">
    ...
    <div class="wai">
      <div class="recom">
        <div class="men_list">
          <span><img :src="VipDetailList.image" alt=""></span>
          <p class="font32 font600">{{VipDetailList.name}}</p>
        </div>
        <ul class="font26">
          <li>{{VipDetailList.businessPoint}}商机点</li>
          <li>{{VipDetailList.product}}次发布产品次数</li>
          <li>{{VipDetailList.purchase}}次发布采购次数</li>
          <li>{{VipDetailList.push}}次/月推送</li>
        </ul>
      </div>
...
<script>
  ...
  export default {
    ...
    computed: {
      nums() {
        return parseInt(this.total / this.VipDetailList.price)
      }
    },
    methods: {
      turnTo(url) {
      ...
      toggle1(index) {
        this.num1 = index;
        this.total = this.cycles[index].ui * this.VipDetailList.price
        this.input1 = null;
      },
      toggle2() {
        this.num1 = null;
      ...
      toggle3() {
        if (this.input1 == null) {
          ...
          this.total = parseInt(this.input1) * this.VipDetailList.price;
        }
      ...
      payVip() {
        ...
        if (this.nums != null && this.nums != 0) {
          var data = {
            userId: value,
            token: this.getkey,
            vipId: this.VipDetailList.guid,
            nums: this.nums,
            payType: 1,
            optUserId:this.userInfo.guid
          };
          this.xhr.post(`${this.subUrl.user}/asset/payVip`, qs.stringify(data)).then((res) => {
            if (res.data.code == 1) {
              console.log(res.data)
              // this.$dialog.toast({mes: '开通成功！', icon: 'success'});
              this.$dialog.toast({mes:"暂不支持微信端购买会员，请移步app购买"});
          ...
          this.$dialog.toast({mes: '请选择月份',timeout:1000});
        }
    ...
    mounted() {
      ...
      let arr = str.split('#')
      this.index1 = parseInt(arr[1])

      let arr1 = JSON.parse(decodeURIComponent(localStorage.getItem('VipDetailList')));
      this.VipDetailList = arr1[this.index1];
