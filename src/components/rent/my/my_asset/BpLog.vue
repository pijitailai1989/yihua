   ...
   <div class="news"  @scroll="scrollHandler" ref="listDom">
       ...
       <ul>
         <li v-for="(item,index) in news" :key="index">
             ...
             <p class="font32 font600"><a v-if="j">+</a><a v-else>-</a>{{item.businessPoint}}</p>
         </li>
       </ul>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getBpLog1(sum){
        var data = {userId: this.organAccount,
        pageNo:this.pageNo,
        pageSize:this.pageSize, token: this.getkey, consumeType:sum};
        this.xhr.post(`${this.subUrl.user}/asset/getBpLog`, qs.stringify(data)).then((res) => {
...
background:#F7F7F9;
  ...
  .news{width:100%;background:white;height:calc( 100vh - .88rem );overflow-y: scroll;
