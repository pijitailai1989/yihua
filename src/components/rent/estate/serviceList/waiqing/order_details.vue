  ...
  <div style='height:100vh'>
    <yd-layout class="layout_my" v-if='showUser'>
      ...
      <div class="flex-row h9 bgwhite bt font32" v-if='(result.repairsBill.status==3) && result.tasks' slot='bottom'>
       ...
       <!-- 接单 -->
      <div class="flex-row jusCenter bgwhite h9 bt" @click="showPay=true" slot="bottom" v-if="result.repairsBill.status == 5 || result.repairsBill.status == 6">
        <span :style='{color:mainColor}' class="font32">接单</span>
      </div>
      <!-- 商家分单 -->
      <div class="flex-row h9 bgwhite bt font32" v-if='(result.repairsBill.status==7) && result.tasks' slot='bottom'>
        <!--<div class="flex1 jusCenter br">-->
          <!--<img style="width:.4rem;height:.4rem;" class="mr1" src="../../../../../assets/images/rent/estate/disagree.png" alt="">-->
  ...
  methods: {
    ...
    acceptOrder() {
      ...
      this.$ajax(
        ...
        res => {
          this.showPay = false;
          this.$dialog.toast({ mes: "已接单", icon: "success" });
          this.getOrderDetail();
        },
        this
      );
