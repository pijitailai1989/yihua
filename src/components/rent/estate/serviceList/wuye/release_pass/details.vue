<template>
  <yd-layout class="layout_my">
    ...
    <div class="mask" v-if="showCode" @click="showCode = false">
     ...
     <div class='buttons' v-if="releasePass.status == 1"><button @click="confirmFang" :style="{background:mainColor}">确 定</button></div>
  ...
  methods: {
      ...
      this.$dialog.confirm({
        ...
        opts: () => {
          ...
          this.$ajax(
            ...
            data,
            res => {
              this.$dialog.toast({ mes: "已放行", icon: "success" });
              this.getReleasePass();
            },
            this
          );
    ...
    confirm() {
      ...
      this.$ajax(
        ...
        data,
        res => {
          this.$dialog.toast({ mes: "已同意放行", icon: "success" });
