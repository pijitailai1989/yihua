<template>
  <yd-layout class="layout_my">
      ...
      <span @click="back" slot="left">
        ...
        <div slot="right" @click="$router.push('/estate/serviceList/wuye/add_complain')" class="edit" style="color: #fff;font-size:.6rem;">+</div>
      </yd-navbar>
          ...
          <div class="list bgwhite pdlr2 mt2" v-for="(item,index) in contentList" :key="index">
            <ul class="font26 flex-row" style="color:#333;border-bottom:1px dashed #ddd;justify-content:space-between">
              ...
              <li>
                ...
                <!-- <span class="pdlr1 state state1" v-if="item.status == 1">上报中</span>
                <span class="pdlr1 state state2" v-if="item.status == 2">接单</span>
                <span class="pdlr1 state state3" v-if="item.status == 3">处理中</span>
                <span class="pdlr1 state state4" v-if="item.status == 4">已完成</span>
                <span class="pdlr1 state state5" v-if="item.status == 5">关闭</span> -->
