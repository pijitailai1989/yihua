<template>
  <yd-layout class="layout_my">
    <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" v-title="'采购销售'" title="采购销售" color="#fff" :bgcolor="mainColor" height=".88rem">
      <router-link :to="'/my/manager'" slot="left"><!--url[url.length-1]-->
        <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
      </router-link>
      <div slot="right" class="edit" style="color: #fff;">
        <router-link :to="'/estate/new_purchase'">
    ...
    <div class="select-tab">
      ...
      <div @click="select(1)" v-bind:class="selectTab==1?'select-tab-active':''">待审批</div>
      <div @click="select(2)" v-bind:class="selectTab==2?'select-tab-active':''">同意</div>
      <div @click="select(3)" v-bind:class="selectTab==3?'select-tab-active':''">拒绝</div>
    </div>
    ...
    <div v-infinite-scroll="loadList" infinite-scroll-disabled="busy" infinite-scroll-distance="0"
      ...
      <div class="item-model" v-for="item, index in arr" v-bind:class="index==0?'top-line':''"
        ...
        <router-link :to="'/estate/order_detail?guid='+item.guid" slot="left"><!--url[url.length-1]-->
        ...
        <div class="show-send-pic show-list-last">
          ...
          <div style="text-align: right">{{item.createTime.split(' ')[0]}}</div>
        </div>
        </router-link>
      </div>
    </div>
...
//      },
      ...
      getSearch() {
        ...
        this.xhr.post(`${this.subUrl.activity}/pasa/applyList?` + qs.stringify(this.data)).then((res) => {
          if (res.data.code == 1) {
...
//            this.$dialog.toast({mes: res.data.msg});
      ...
      getAllArr() {
        var data = {token: this.getkey, organId: this.organInfo.guid};
        // {params: {type: 'pulldown', page: this.page}
        this.xhr.post(`${this.subUrl.activity}/pasa/applyList?token=` + this.getkey + '&pageNo=' + this.page + '&pageSize=10').then((res) => {
          if (res.data.code == 1) {
...
<style scoped>
  ...
  .black-bg{position: fixed;background: black;opacity: 0.3;width: 100%;top: 6.3em;height: 100%}
