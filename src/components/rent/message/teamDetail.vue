<template>
  ...
  <yd-layout class="edit" v-title="'联系人'">
    <!--<yd-navbar height=".88rem" v-show="!userAgent.isWX" title="联系人" bgcolor="#0093d2" color="#fff">-->
      ...
      <!--</span><br>-->
    <!--</yd-navbar>-->
    <div v-for="item,index in data.teams" v-if="item.teamId==teamId">
      <div class="team-detail-name text-center">
      <span slot="left" @click="back" class="pull-left" color="#fff;padding-right:.2rem" style="color: #fff;">
        <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
      </span><br><br>
        <img class="m-right l-right team-avatar" :src="item.avatar!=''?item.avatar:data.myInfo.avatar"><br>
        {{item.name}}
      </div>
      <div class="team-detail-people">
        成员 <span class="pull-right">{{item.memberNum}}人</span><br>
        <div><img class="members-pic" v-for="it,index1 in data.nowMenber" v-if="index1<6" :src="it.account*1!=data.myInfo.account*1?friendImg(it.account):data.myInfo.avatar" alt=""></div>
      </div>
      <div style="margin-top: 1em">
        <div class="team-detail-people" @click="lookHistory">消息记录<span class="pull-right"> <img class="m-right" src="../../../assets/images/rent/my/enter.png"></span><br></div>
        <!--<div class="team-detail-people">图片记录 <span class="pull-right"> <img class="m-right" src="../../../assets/images/rent/my/enter.png"></span><br></div>-->
        <div class="team-detail-people" v-if="item.owner*1==data.myInfo.account*1" @click="clearRecord">清除消息记录 <span class="pull-right"> <img class="m-right" src="../../../assets/images/rent/my/enter.png"></span><br></div>
        <div class="team-detail-people" v-if="item.owner*1==data.myInfo.account*1" @click="clearTeam">解散群 <span class="pull-right"> <img class="m-right" src="../../../assets/images/rent/my/enter.png"></span><br></div>
        <div class="team-detail-people" v-if="item.owner*1!=data.myInfo.account*1&&couldDel(item.teamId)" @click="leaveTeam">退出 <span class="pull-right"> <img class="m-right" src="../../../assets/images/rent/my/enter.png"></span><br></div>
      </div>
    </div>
  </yd-layout>
  <!--</div>-->
    ...
    methods: {
      couldDel(data){
        ...
        for(var i=0;i<this.data.teams.length;i++){
          console.log(this.data.teams[i].serverCustom)
          if(data*1==this.data.teams[i].teamId*1&&this.data.teams[i].serverCustom){
            console.log(this.data.teams[i].type)
            flag=false
          }
        }
        return flag;
      ...
      clearTeam(){
        this.nim.dismissTeam({
          teamId: this.teamId,
          done: this.dismissTeamDone,
        });
      },
      leaveTeam(){
        this.nim.leaveTeam({
          teamId: this.teamId,
          done: this.leaveTeamDone
        });
      },
      ...
      lookHistory(){
        this.$router.push('/message/chat_history?teamId='+this.teamId);
      },
      friendImg(data) {
        var img = ''
        for (var i = 0; i < this.data.users.length; i++) {
          if (this.data.users[i].account * 1 == data*1) {
            img = this.data.users[i].avatar;
            console.log(img)
          }
          else{
            img=this.data.myInfo.avatar
          }
        }
        return img
      },
      ...
      getTeamMembersDone(error, obj) {
        ...
        console.log(obj);
        if(!error){
          this.data.nowMenber=obj.members;
          localStorage.setItem('chatData', encodeURIComponent(JSON.stringify(this.data)));
        }
      ...
      getTeamDetail(){
        this.nim.getTeam({
          teamId: this.teamId,
          done: this.getTeamDone
        });
      },
      getTeamDone(error, obj) {
        console.log(error);
        console.log('获取群' + (!error?'成功':'失败'));
        if(!error){
          this.data.showTeam=obj;
          localStorage.setItem('chatData', encodeURIComponent(JSON.stringify(this.data)));
        }
      },
      sureAdd(){
...
//         this.data.teamMembers[teamId] = this.nim.cutTeamMembers( this.data.teamMembers[teamId], members.invalid);
    ...
    mounted() {
      ...
      this.teamId=this.$route.query.id.split('-')[1];
