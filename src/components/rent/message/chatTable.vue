<template>
  ...
  <div style="background: #f9fcff;margin-bottom: -0.5px;height: 100%">
    <div style="width: 100%;position: relative;height: 100%;">
      <yd-navbar height=".88rem" v-show="!userAgent.isWX" :title="title" bgcolor="#0093d2" color="#fff">
      <span slot="left" @click="back" style="color: #fff;">
        <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
      </span>
        <span slot="right" @click="chooseSuccess()" v-if="teamId&&teamId!=''&&!isHistory"
              style="color:#fff;margin-top:-1em">
        <yd-icon name="search"></yd-icon>
        详情
      </span>
        <span slot="right" @click="toHistory()" v-if="type.split('-')[0]=='p2p'&&!isHistory"
              style="color:#fff;margin-top:-1em">
        <yd-icon name="search"></yd-icon>
        历史记录
      </span>
      ...
      <div class="bottom-input" v-if="!isHistory">
        ...
        <!--</textarea>-->
        <p id="p" class="needsclick" v-bind:focus="onFocus()" v-on:blur="onBlur" contenteditable="true"></p>
        <a style="position: absolute;bottom: 0;hieght:0.88rem;margin: 0;border: none;max-height: 0.88rem;">
          ...
          <span data-v-477b3f6c="" class="u-editor-icon pull-right">
