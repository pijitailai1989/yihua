<template>
  <yd-layout class="edit">
    <yd-navbar height=".88rem" slot='navbar' v-show="!userAgent.isWX" v-title="'我的客户'" title="我的客户" :bgcolor="mainColor" color="#fff">
      <span class="addClickArea" slot="left" @click="back" style="color: #fff;">
        <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
      </span>
      <span class="addClickArea" @click="turnTo('/supply/purchase/blacklist#2')" slot="right" style="color:#fff">黑名单</span>
    </yd-navbar>
    <div class="search">
             <ul>
               <li @click="toggle"><span style="color:#999;margin-right:.1rem;">全部</span><yd-icon name='toDown' size='.2rem' custom color="#999"></yd-icon></li>
    ...
    <div class="lists" @scroll="scrollHandler" ref="listDom">
             <ul class="libList" v-for="(item,index) in libraryList" :key="index">
                ...
                <li @click="turnTo('/supply/purchase/supplier_details',index)">
                     <p>
    ...
    <div class="checkAll">
           <ul>
              ...
              <li>
                <span class="font30" @click="delOrblackSupplier(2)">删除</span>
              </li>
           </ul>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      turnTo(url,index){
        ...
        this.libraryList[index].titleText='客户详情';
