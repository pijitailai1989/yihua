<template>
  <yd-layout class="edit">
    <yd-navbar height=".88rem" slot='navbar' v-show="!userAgent.isWX" v-title="'采购收藏'" title="采购收藏" :bgcolor="mainColor" color="#fff">
    ...
    <div class="search">
             <ul>
               ...
               <li><yd-search placeholder="输入搜索关键词" :on-submit="submitHandler"></yd-search></li>             
             </ul>
    ...
    <div class="lists" @scroll="scrollHandler" ref="listDom">
             <ul class="libList" v-for="(item,index) in libraryList" :key="index">
                ...
                <li @click="turnTo('/supply/purchase/details/'+item.type+'/'+item.poId)">
                     <p>
                       <span class="font32 textHidden" style="width:4rem">【{{item.type1}}】{{item.title}}</span>
                     </p>
                     <p>
                       <span>深圳市中荣科技无责任公司</span>
                     </p>
                ...
                <li>
                    <p>
                        <span :style="{border:'1px solid '+mainColor,color:mainColor}">正常</span>
                    </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitHandler(value) {
                this.$dialog.toast({mes: `搜索关键字：${value}`});
                this.parameter.pageNo=1;
