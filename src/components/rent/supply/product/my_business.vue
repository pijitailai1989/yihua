<template>
  <yd-layout class="edit">
    <yd-navbar height=".88rem" slot='navbar' v-show="!userAgent.isWX" v-title="'我的商机'" title="我的商机" :bgcolor="mainColor"
               color="#fff">
      <span class="addClickArea" slot="left" @click="back" style="color: #fff;">
        <yd-navbar-back-icon color="#fff"></yd-navbar-back-icon>
      </span>
      <span class="addClickArea" @click="turnTo('/supply/product/purchase_collection')" slot="right"
            style="color: #fff;">我的收藏</span>
    </yd-navbar>
    <div class="search" style="border-bottom:1px solid #e4e4e4;margin-bottom:.2rem">
      <ul>
        <li @click="toggle"><span style="color:#999;margin-right:.1rem;">日期</span>
    ...
    <div class="tab">
      <ul>
        <li @click="leftTab"><span style="color:#999">&nbsp;<&nbsp;</span></li>
        <li>
          <yd-tab :active-color="mainColor" style="width:100%;" :callback="callback">
            <yd-tab-panel v-for="(item,index) in list" :key="index" :tabkey='index+1' :label="item"
                          :active="index==0?true:false"></yd-tab-panel>
          </yd-tab>
        </li>
        <li @click="rightTab"><span :style="{color:mainColor}">&nbsp;>&nbsp;</span></li>
      </ul>
    </div>
    ...
    <div class="lists" @scroll="scrollHandler" ref="listDom">
      <ul class="libList" v-for="(item,index) in libraryList" :key="index">
        <li
          @click="turnTo('/supply/product/level_list/'+parameter.type+'/'+item.guid+'?title='+item.title+'&status='+item.state1)">
          <p>
            <span class="font32 textHidden" style="width:4.5rem">{{item.title}}</span>
          </p>
          <span class="font26" style="color:#A7A7A7;">
						     <i><img src="../../../../assets/images/rent/supply/riqi.png" alt=""></i>{{item.createTime}}
					           </span>
        </li>
        <li>
          <p>
            <span class="font24" :style="{border:'1px solid '+mainColor,color:mainColor}">{{item.state1}}</span>
          </p>
...
<script>
  ...
  export default {
    ...
    watch: {
      'parameter.type': 'opportunityList'
    },
    methods: {
      turnTo(url) {
      ...
      opportunityList() {
        ...
        this.xhr.post(`${this.subUrl.bd}/opportunity/opportunityList`, qs.stringify(data)).then((res) => {
          if (res.data.code == 1) {
