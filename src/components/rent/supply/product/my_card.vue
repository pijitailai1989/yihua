<template>
  <yd-layout class="edit">
    ...
    <div class="declares" style="margin-bottom:.2rem;">
        <ol>
            <li sytle="color:#666"><p>名称:{{organName}}</p><span @click="turnTo('/supply/product/my_shop?userId='+pm.userId)" :style="{color:mainColor,border:'1px solid '+mainColor}">进入店铺</span></li>
        </ol>
        <ul>
            <!-- <li>
                <p>头像</p>
    ...
    <div class="text" style="margin-bottom:.2rem;">
      ...
      <div class="images1">
          ...
          <yd-lightbox v-else  :num="imgUrl2.length">
              <yd-lightbox-img :onerror="logo" v-for="(item,index) in imgUrl2"  :key="index" :src="item.path"></yd-lightbox-img>
          </yd-lightbox>
      </div>
    </div>

    <yd-cell-group v-show="show">
