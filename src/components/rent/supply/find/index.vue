	...
	<div class="template">
		<yd-layout class="find">
          ...
          <div class="flex_row" ontouchmove='return false'>
            <div class="flex_row_item" :class="{allActive:allActive[0]}" :style="allActive[0]?`color:${mainColor}`:``" @click="setActive(0)">
            ...
            <div class="flex_row_item" :class="{allActive:allActive[1]}" :style="allActive[1]?`color:${mainColor}`:``" @click="setActive(1)">全部分类
              ...
              <div v-if="show0">
                 ...
                 <div class="hierarchy">
                   ...
                   <ul v-if="districtList3.length>0" style="background:#F8F9FA;">
                       ...
                       @click="getDistrictList(index,item.guid,4,item.dictDataName)">{{item.dictDataName}}</li>
                   </ul>
                 </div>
              </div>
              <div v-if="show1">
          ...
          <ul v-for="(item,index) in list" :key="index" style="border-bottom:1px solid #ddd;">
				   <li>
             <i v-if="item.type==1"><img src="../../../../assets/images/rent/supply/biao.png" alt=""></i>
             <i v-if="item.type==2"><img src="../../../../assets/images/rent/supply/gou.png" alt=""></i>
             <i v-if="item.type==3"><img :onerror="imgs" :src="item.img" alt=""></i>
           </li>
           <li v-if="item.type==3" @click="turnTo('/supply/find/details/'+item.guid)">
				   ...
				   <li>
					   <i v-if="item.isCollection==1" @click="cancelCollection(item.type,item.guid,index)"><img src="../../../../assets/images/rent/supply/shou1.png" alt=""></i>
					   <i v-else @click="collection(item.type,item.guid,index)"><img src="../../../../assets/images/rent/supply/shou2.png" alt=""></i>
				   </li>
			   </ul>
			   <div  class="Section">
        ...
        this.xhr.post(`${this.subUrl.bd}/market/marketIndex`, qs.stringify(data) ).then((res) => {
          ...
          if (res.data.code == 1) {
            ...
            res.data.data.list.forEach(element => {
               ...
               if(element.img!=''){
                 element.img=element.img.split(',')[0]
               }
            });
            }
...
<style scoped>
  ...
  .show{position:fixed;width:100%;height:100vh;
  background: rgba(0, 0, 0, 0.8);z-index:999;}
  .show>div{width:100%;height:6rem;background: white;color:#999;}
  .show .navigation{width:100;height:.72rem;display: flex;justify-content: space-between;
  padding:0 .2rem;align-items: center;border-bottom:1px solid #ddd;}
  .show .navigation>p:nth-child(1)>span:last-child{color:#000;}
  .show .navigation>p:nth-child(2)>span:last-child{margin-left:.5rem;}

  .show .hierarchy{width:100%;height:5.28rem;display: flex;}
  .show .hierarchy>ul{flex:1;height:5.28rem;overflow-y:scroll; border-right: 1px solid #ddd;}
  .show .hierarchy li{height:.7rem;padding:0 .1rem;line-height: .7rem;display: flex;justify-content: space-between;
  border-bottom:1px solid #ddd;
  white-space: nowrap;
  overflow: hidden;
  ...
  .content{width:100%;height: calc( 100vh - 5.1rem );overflow-y: scroll;}
  .content>ul{width:100%;display: flex;justify-content: space-around;align-items: center;
  background: white;margin-bottom:.3rem;}
  .content>ul>li{display: flex;flex-flow:column;}
  .content>ul>li:nth-child(1){width:2rem;height:2rem;justify-content: center;
  align-items: center;}
  .content>ul>li:nth-child(1)>i{width:1.6rem;height:1.6rem;}
  .content>ul>li:nth-child(1)>i>img{width:100%;;height:100%;}
  .content>ul>li:nth-child(2){height:2rem;flex:1;justify-content:center;}
  .content>ul>li:nth-child(2)>p{}
  .content>ul>li:nth-child(2)>p:nth-child(1){font-weight: 600;font-size:.3rem}
  .content>ul>li:nth-child(2)>p:nth-child(2){color:#999;font-size:.26rem;margin:.05rem 0;}
  .content>ul>li:nth-child(2)>p:nth-child(3){font-size:.24rem;margin:.05rem 0;display: flex;}
  .content>ul>li:nth-child(2)>p:nth-child(3)>span{display: flex;margin-right: .2rem;
  width:.8rem;height:.3rem;justify-content: center;align-items:center;}
  .content>ul>li:nth-child(2)>p:nth-child(4){color:#999;font-size:.24rem;display: flex;
  flex-flow:row;align-items: center;margin-top:.05rem;}
  .content>ul>li:nth-child(2)>p:nth-child(4)>i{width: .3rem;height:.3rem;margin-right:.1rem;}
  .content>ul>li:nth-child(2)>p:nth-child(4)>i>img{width:100%;height:100%;}
  .content>ul>li:nth-child(3){width:1.4rem;height:2rem;align-items:center;justify-content: center;}
  .content>ul>li:nth-child(3)>i:nth-child(1){width:1.2rem;height:1.2rem;display: flex;align-items: center;
  justify-content: center;border-left:1px solid #F1F1F1;}
  .content>ul>li:nth-child(3)>i:nth-child(1)>img{width:.4rem;height:.4rem;}
