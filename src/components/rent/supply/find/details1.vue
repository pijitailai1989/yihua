<template>
  <yd-layout class="layout_my">
      <yd-navbar class="header" slot="navbar" v-show="!userAgent.isWX" v-title="'详情'" title="详情" color="#fff" :bgcolor="mainColor" height=".88rem">
     ...
     <div class="organInfos">
      <div class="info">
         <ul>
           <li><span>标题</span><p>{{productInfo.title}}</p>
           <a class="font24" :style="{border:'1px solid'+mainColor,color:mainColor}">{{productInfo.state}}</a></li>
           <li><span>供应商</span><p>{{organInfoDto3.organName}}</p>
           <a @click="turnTo('/supply/product/my_card?guid='+productInfo.userId+'&oName='+organInfoDto3.organName)" 
           :style="{color:mainColor}">【商家名片】</a></li>
           <li><span></span><p><i style="background:#C8E7FF;color:#61BBFF;" v-if="organInfoDto3.vipType!=''">{{organInfoDto3.vipType}}</i>
           ...
           <li><span>发布时间</span><p>{{productInfo.createTime}}</p></li>
           <li><span>联系人</span><p>{{organInfoDto3.userName}}</p>
           <a v-if="productInfo.isLookPhone==0&&productInfo.userId!=userId" @click="openConfrim" :style="{border:'1px solid'+mainColor,color:mainColor}">
           ...
           <a v-if="productInfo.isLookPhone==1&&productInfo.userId!=userId" @click="openC" :style="{border:'1px solid'+mainColor,color:mainColor}">
             <i v-if="mainColor=='#23ac38'"><img src="../../../../assets/images/rent/supply/phone1.png" alt=""></i>
             <i v-else><img src="../../../../assets/images/rent/supply/phone.png" alt=""></i>联系(已查看)</a>
           </li>
           <li><span>所在地</span>
           <p>{{organInfoDto3.organAddress}}</p>
           <a :style="{border:'1px solid'+mainColor,color:mainColor}" v-show="2==1">
             ...
             地图</a></li>
           <li><span>最小采购</span><p>{{productInfo.minNumber==0?'不限':productInfo.minNumber}}</p></li>
      ...
      <div class="group_buying" v-if="productInfo.isGroup!=0">
          ...
          <div>
              <ul>
                  ...
                  <li><span style="margin-left:.4rem;color:#999">{{priceList[1]}}</span><a>服务区域 {{productInfo.scopeValueStr}}</a></li>
              </ul>
          </div>
      </div>
    </div>
    ...
    <div class="text">
      ...
      <ul>
          <li v-show="item.parameterName!=''" v-for="(item,index) in productParameterList" :key="index">{{item.parameterName}}：{{item.parameterValue}}</li>
      </ul>
      ...
      <div class="images" >
            ...
            <yd-lightbox>
                <yd-lightbox-img v-for="(item,index) in imgList" :key="index" :src="item"></yd-lightbox-img>
            </yd-lightbox>
        </div>
    </div>
    ...
    <div class="alertText" v-show="alertT">
      <ul>
          <li class="a" v-show="this.bm.type==2||this.bm.type==1"><p class="font28">姓名：</p><span><input v-model="bm.name" type="text" placeholder="请填写"></span></li>
          <li class="a" v-show="this.bm.type==2||this.bm.type==1"><p class="font28">电话：</p><span><input v-model="bm.phone" type="number" placeholder="请填写"></span></li>
          <li class="a" v-show="this.bm.type==2||this.bm.type==1"><p class="font28">验证码：</p><span><input v-model="bm.code" type="number" placeholder="请填写"></span></li>
          <li class="a" v-show="this.bm.type==2||this.bm.type==1"><p class="font28">地址：</p><span><input v-model="bm.address" type="text" placeholder="请填写"></span></li>
          <li class="a b">
...
.declares>ol li:nth-child(1){
  ...
  font-size:.28rem;padding:.2rem 0;}
  ...
  .info ul li>a>i{display: flex;width: .3rem;height:.3rem;margin-right:.05rem;}
  ...
  .text>.ttt{width:100%;border-bottom:1px solid #F3F6F5;
  padding:.1rem .3rem;font-size: .28rem;}
  .text>ul{width:100%;}
  ...
  .images{width:90%;margin-top:.2rem;display: flex;margin-bottom:.3rem;}
  .images>div{width:100%;display: flex;flex-flow:row;justify-content:flex-start;align-items:center;}
  .images>div>img{width:1.2rem;height:1.2rem;border-radius: .1rem;margin-right: .1rem;}
...
.textar>div>i.aa{flex:1;}
  ...
  .alertText{z-index:999;width:100%;position:fixed;height:100vh;background: rgba(0,0,0,.1);top:0;left:0;display: flex;justify-content: center;align-items: center;}
  .alertText>ul{width:6.5rem;background: white;padding:.2rem .3rem;
  ...
  .alertText ul li:last-child>button{font-size:.3rem;color:white;
  padding:.15rem .3rem;border:none;border-radius: .1rem;}
  .alertText ul li.a>span>textarea{border:none;width:100%;height:100%;}
</style>
